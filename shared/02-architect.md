...
> **Model: Opus 4.6** — architecture decisions require deep trade-off reasoning. Before starting, tell the user: "Before we begin, switch to **Opus 4.6** in the model picker (bottom-left of the chat panel). Architecture needs the deepest reasoning model. Let me know when you've switched and I'll start." **Wait for the user to confirm before proceeding.**
...
> "Saved to `docs/02-architecture.md`. When you're ready, continue with **`.workflow/shared/03-spec-system.md`** to write the product/system spec."
...
**Wait for the user's input.** If yes, tell the user: "Switch to **Composer 2** for the HTML export — it's pure templating work and much cheaper. Let me know when you've switched." Wait for confirmation, then generate `docs-pdf/02-architecture.html` following the styling rules in `.workflow/shared/cursor-rules/docs-pdf-export.mdc`. Ensure `docs-pdf/` is gitignored. After the export, remind the user to switch back to the previous model. If no, move on.
