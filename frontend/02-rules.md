...
> **Model: Opus 4.6** — track rules require reasoning about project-wide consistency. Before starting, tell the user: "Before we begin, switch to **Opus 4.6** in the model picker (bottom-left of the chat panel). Setting rules needs careful reasoning. Let me know when you've switched and I'll start." **Wait for the user to confirm before proceeding.**
...
If the user wants IDE assistance aligned with these rules, **optionally** add or update files under `.cursor/rules/` to mirror the most important constraints (keep them short and actionable).

> "Should I also add a short `.cursor/rules/` file that points at these conventions?"
